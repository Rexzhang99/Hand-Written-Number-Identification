...
# }
#
# log_likelihood(X_mat, y, beta_hat)
#
...
#     delta <-
#       t(X_mat) %*% as.matrix(residual, ncol = nrow(X_mat)) * (1 / nrow(X_mat))
#     beta_hat <- beta_hat - (lr * delta)
#   }
...
# }
#
# # logistic_reg(X, y, 1000, lr)


#Step 3 - Softmax Regression####
...
neuralnetwork <-
           ...
           validation_data = training_data)
  {
    ...
    listw <-
      sizes[1:length(sizes) - 1] # Skip last (weights from 1st to 2nd-to-last)
    listb <-
      sizes[-1]  # Skip first element (biases from 2nd to last)
    ...
    weights <- lapply(seq_along(listb),
                      function(idx) {
                        c <- listw[[idx]]
                        r <- listb[[idx]]
...
SGD <-
  function(training_data,
    ...
    for (j in 1:epochs) {
      ...
      mini_batches <-
        split(training_data, ceiling(seq_along(training_data) / mini_batch_size))
      # Feed forward (and back) all mini-batches
      for (k in 1:length(mini_batches)) {
        # Update biases and weights
        res <-
          update_mini_batch(mini_batches[[k]], lr, C, sizes, num_layers, biases, weights)
        biases <- res[[1]]
        weights <- res[[-1]]
      }
      # Logging
...
update_mini_batch <-
           ...
           weights)
  {
    ...
    nabla_b <- lapply(seq_along(listb),
                      function(idx) {
                        r <- listb[[idx]]
                        matrix(0, nrow = r, ncol = 1)
                      })
    nabla_w <- lapply(seq_along(listb),
                      function(idx) {
                        c <- listw[[idx]]
                        r <- listb[[idx]]
    ...
    for (i in 1:nmb) {
      ...
      nabla_b <-
        lapply(seq_along(biases), function(j)
          unlist(nabla_b[[j]]) + unlist(delta_nabla_b[[j]]))
      nabla_w <-
        lapply(seq_along(weights), function(j)
...
backprop <- function(x, y, C, sizes, num_layers, biases, weights)
{
  ...
  for (f in 1:length(biases)) {
    ...
    w_a <- w %*% activation
    b_broadcast <-
      matrix(b, nrow = dim(w_a)[1], ncol = dim(w_a)[-1])
    z <- w_a + b
    zs[[f]] <- z
    activation <- sigmoid(z)
    activations[[f + 1]] <-
      activation  # Activations already contain one element
  }
  # Second:
  ...
  delta <-
    cost_delta(
      method = C,
      z = zs[[length(zs)]],
      a = activations[[length(activations)]],
      y = y
    )
  nabla_b_backprop[[length(nabla_b_backprop)]] <- delta
  nabla_w_backprop[[length(nabla_w_backprop)]] <-
    delta %*% t(activations[[length(activations) - 1]])
  ...
  if (num_layers > 2) {
    for (k in 2:(num_layers - 1)) {
      ...
      delta <-
        (t(weights[[length(weights) - (k - 2)]]) %*% delta) * sp
      nabla_b_backprop[[length(nabla_b_backprop) - (k - 1)]] <-
        delta
      testyy <- t(activations[[length(activations) - k]])
      nabla_w_backprop[[length(nabla_w_backprop) - (k - 1)]] <-
        delta %*% testyy
    }
  }
...
evaluate <- function(testing_data, biases, weights)
{
  ...
  truths <- lapply(test_y, function(x)
    which.max(x))
  # Accuracy
  correct <- sum(mapply(function(x, y)
    x == y, pred, truths))
  total <- length(testing_data)
  print(correct / total)
  # Confusion
  res <-
    as.data.frame(cbind(t(as.data.frame(pred)), t(as.data.frame(truths))))
  colnames(res) <- c("Prediction", "Truth")
  error.table=table(as.vector(res$Prediction), as.vector(res$Truth))
  print(error.table)
  list(error.table,correct / total)
}

#Common math functions##########
cost_delta <-
  function(method, z, a, y) {
    if (method == 'ce') {
      return (a - y)
    }
  }
...
train_test_from_df <- function(df,
                               predict_col_index,
                               train_ratio,
                               shuffle_input = TRUE,
                               scale_input = TRUE)
{
  ...
  dmy <- function(df)
  {
    ...
    if (length(factor_columns) > 0)
    {
      ...
      dmy_enc <- model.matrix(
        ~ . + 0,
        data = df[factor_columns],
        contrasts.arg = lapply(df[factor_columns], contrasts, contrasts =
                                 FALSE)
      )
